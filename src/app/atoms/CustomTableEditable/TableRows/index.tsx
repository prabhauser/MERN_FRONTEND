...
const UserlistBody = (props: Props) => {
    ...
    const [periodArray, setPeriodArray] = useState<any>([{ name: '', code: '' }]);
    const [secondValue, setSecondvalue] = useState('');
    const [secondArray, setSecondArray] = useState<any>([{ name: '', code: '' }]);

    useEffect(() => {
        if (row?.Remark) {
            setPeriodArray(row?.Remark);
            setPeriodvalue(row?.selected);
            setSecondArray(row?.second);
            setSecondvalue(row?.selected);
        }
    }, []);
    ...
    const changeDropdown = (event: any) => {
        ...
        checkDrodownSelection(row, event);
        setPeriodvalue(event);
    };
    const changeDropdown2 = (event: any) => {
        console.log(row, event);
        checkDrodownSelection(row, event);
    ...
    const displayArray = orderList.map((ele: any, index: number) => {
        ...
        if (ele === 'Remark' && editable) {
            return (
                <TableCell align="left" key={cellKey} className="remarkRow">
                    <DropDown
                        onChange={(event: any) => changeDropdown(event.target.value)}
                        name={''}
                        items={periodArray}
                        label={''}
                        value={periodValue !== '' ? periodValue : ''}
                    />
                </TableCell>
            );
        } else if (ele === 'Remark') {
        ...
        if (ele === 'Second' && editable) {
            return (
                <TableCell align="left" key={cellKey} className="remarkRow">
                    <DropDown
                        onChange={(event: any) => changeDropdown2(event.target.value)}
                        name={''}
                        items={secondArray}
                        label={''}
